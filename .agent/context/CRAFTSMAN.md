...
**BEFORE** performing any task, the AI MUST proactively:
1. Read `.agent/context/` (Conventions, Domain, etc.) to align with project standards.
2. Follow `.agent/workflows/daily-dev.md` for execution (or `/health-check` if fixing/scanning bugs).
...
- **Modularity & Encapsulation:** Small, well-encapsulated services. Each service should be technology-agnostic where possible.
- **Hexagonal Thinking:** Use Ports & Adapters terminology. `Domain` defines the "What" (Ports/Interfaces), `Infrastructure` defines the "How" (Adapters).
- **Mentorship Mode:** Don't just act as a manual laborer; act as a Senior Mentor.
  - **Annotation:** Add short `// Logic:` or `// Why:` comments for design choices.
  - **Partial Implementation:** Offer "Challenge Mode" (e.g., TODO placeholders) for critical logic blocks for the USER to implement and learn.
  - **Code Walkthrough:** Provide brief syntax explanations for Java-specific features (Records, Streams, JPA annotations).
- **Evaluate Design Options:** Consider multiple architectural alternatives (REST vs event-driven, different patterns) based on scalability, maintainability, and testability before deciding.
- **Apply Design Patterns Judiciously:** Use patterns (Factory, Observer, Proxy) only when they provide clear, measurable benefit — avoid overengineering.
- **DDD Tactical Patterns:** Apply Entities, Value Objects, Aggregates, Repositories, and Domain Services accurately to reflect the business domain.
- **Balance Trade-offs:** Document rationale when favoring one principle over another (e.g., sacrificing DRY for better service isolation).
