...
**Test Chuẩn Chỉ (2026-09-01): test pyramid toàn dự án — P1 Backend Integration (Testcontainers thật) + P2 FE Component (RTL) + P3 Contract (lightweight shared-fixture, không Pact broker) + P4 E2E main user flow + P5 CI pipeline. Toàn bộ đã xanh: `mvn test` **BUILD SUCCESS (auth 47 / playlist 29 / track 29 / search 22)** + FE **61 tests / 11 files** + lint + typecheck sạch + **Playwright E2E 6/6** (gating 2 + auth 2 + main-flow 2). E2E P4 **phát hiện bug thật**: `audioUrl` từ backend đã kèm `/api/v1` → `resolveApiUrl` gắn thêm base tạo URL kép `/api/v1/api/v1/...` khiến Player không phát được track (fix `resolveApiUrl` idempotent). P3 **lộ contract drift âm thầm**: search bootstrap parse `GET /tracks` thành `TrackPayload` (7 field, id String) trong khi provider trả `TrackResponse` (9 field, id UUID) — chạy được chỉ nhờ Jackson lenient.**
...
- Vitest: **61 test xanh (11 test files)** (search-logic, SearchBar, services/api searchService + authService, PlayerProgress, usePlayerStore, useAuth, validation/auth, **login page component — P2**, **resolveApiUrl regression — P4**) + lint + `tsc --noEmit` sạch
- **Test chuẩn chỉ (2026-09-01):** P1 Backend Integration (block lớn nhất trước đây = 100% Mockito, không Spring context, không DB thật) → Testcontainers: `@DataJpaTest` Flyway↔JPA validate trên **Postgres 16 thật** (auth/playlist/track) + **Elasticsearch 8.15.3 thật** (search), `@WebMvcTest` contract HTTP-layer (regression-guard `/me` double-wrap). P2 FE Component RTL (login page 4 test) + fix 2 authService test lỗi do unwrap refactor. P5 CI: `.github/workflows/ci.yml` (mỗi PR/push: backend `mvn test` + FE lint/typecheck/vitest) + `e2e.yml` (workflow_dispatch full-stack E2E).
...
- `track-service` (port **8085**) — Clean Arch: domain (`Track`, `TrackAudioFile`, `TrackAudioRange`, events, `TrackRepository`/`TrackAudioRepository` port) / application (Create/GetByIds-batch/Update/List-all/**UploadAudio/GetAudio** usecase) / infrastructure (JPA adapter/mapper, **MinIO client**, **Kafka publisher**, exception handler) / presentation (controller)
  ...
  - **Test: 29 xanh — 26 unit (Create 5, GetByIds 3, Update 3, List 1, UploadAudio 4, GetAudio 2, Mapper 2, events 6) + 2 integration (TrackJpaRepository, Testcontainers Postgres) + 1 P3 contract provider (TrackListAllContractProviderTest)**
- `search-service` (port **8086**) — Clean Arch: domain (`TrackSearchDocument`, `TrackSearchRepository` port) / application (Index/Remove/Search track usecase) / infrastructure (**Elasticsearch adapter** `TrackElasticsearchRepository`, **Kafka consumer** `TrackEventConsumer` group `search-service-group`, **bootstrap** `TrackIndexBootstrap` reindex lúc boot, `RestTrackBootstrapFetcher` GET track-service list-all) / presentation (`SearchController`)
  ...
  - **Test: 22 xanh — 16 unit (Index 4, Remove 2, Search 4, Consumer 4, Bootstrap 2) + 4 integration (TrackElasticsearchRepository trên ES 8.15.3 container xpack security tắt, 2026-09-01) + 2 P3 contract consumer (TrackListAllContractConsumerTest)**
  ...
  - **Test: 29 xanh — 27 unit (LexoRank 8, Scheduler 2, AddTrack 5, Reorder 5, GetPlaylistById 2, GetTracks 2, ListPlaylists 2, Rebalance 1) + 2 integration (PlaylistTrackJpaRepository, Testcontainers Postgres)**
...
12. ✅ **Test chuẩn chỉ toàn dự án — P1/P2/P3/P4/P5 (2026-09-01)** — lấp block lớn nhất (backend trước đây **100% Mockito**, không Spring context, không DB thật):
    - **P1 Backend Integration (Testcontainers):** `@DataJpaTest` Flyway↔JPA `validate` trên **Postgres 16 thật** (auth **UserJpaRepository** 4 test, playlist **PlaylistTrackJpaRepository** 2 test, track **TrackJpaRepository** 2 test) + **Elasticsearch 8.15.3 thật** (search **TrackElasticsearchRepository** 4 test — mapping index + multi_match fuzziness + index/remove round-trip; ES 8.x bật HTTPS/xpack mặc định → `.withEnv("xpack.security.enabled","false")` để RestClient HTTP kết nối) + **`@WebMvcTest` AuthController** 4 test (contract HTTP-layer, `addFilters=false`, regression-guard `/me` single-wrap). Base class `AbstractPostgresIntegrationTest` chung mỗi service. **`./mvnw test` BUILD SUCCESS toàn backend: auth 47 / playlist 29 / track 28 / search 20.**
    - **P2 FE Component (RTL):** `login/page` 4 test (form render/submit → `useLogin().mutate`, mfaRequired → bước 2FA + `verify2faLogin(mfaToken, code)`, Google button) + **fix 2 authService test** lỗi do unwrap refactor trước đó + **fix type latch** `verifyMock` thiếu `data`. 
    - **P3 Contract test (service↔service):** chọn **lightweight shared-fixture** thay vì Pact broker (Pact cần broker infra riêng + `maven-pact-plugin`, tỉ lệ lợi ích/rủi ro thấp cho 1 cạnh HTTP nội bộ). Contract duy nhất thật sự là **search→track `GET /api/v1/tracks` list-all** (bootstrap reindex). Fixture chung `backend/contracts/track-list-all.contract.json` + 2 phía tự assert: **provider** `TrackListAllContractProviderTest` (@WebMvcTest so mảng `data` với fixture) + **consumer** `TrackListAllContractConsumerTest` (stub HttpServer serve fixture → `RestTrackBootstrapFetcher.fetchAll()` parse 7 field `TrackPayload` nguyên vẹn). **Lộ contract drift âm thầm:** consumer parse `TrackResponse` (9 field, id UUID, thêm createdAt/updatedAt, bỏ key null artworkUrl) thành `TrackPayload` (7 field, id String) — chạy được chỉ nhờ Jackson lenient bỏ field thừa; test này chốt 7 field consumer cần, chống bootstrap âm thầm index rỗng nếu producer đổi field. 
    - **P4 E2E main user flow:** `frontend/e2e/main-flow.spec.ts` — mở Chill Mix → click dòng track → Player phát (nút **Pause**) + seek slider (ArrowRight tăng aria-valuenow) → search "odesza" → click kết quả → Player phát track seeded. Mỗi test tự tạo account qua API + login UI. **Full Playwright 6/6 xanh** (gating 2 + auth 2 + main-flow 2). **Bug thật bắt được:** `audioUrl` từ backend (seed) đã kèm `/api/v1`, `resolveApiUrl` gắn thêm BASE_URL (`...:9000/api/v1`) → URL kép `/api/v1/api/v1/.../audio` → stream 401/format error, **mọi track không phát được sau khi click**. Fix `resolveApiUrl` idempotent (path `/api/v1/*` → chỉ gắn origin) + **4 unit test regression** `lib/__tests__/apiClient.test.ts`. **FE 61 test / 11 files xanh + lint + typecheck sạch.**
    - **P5 CI pipeline:** `.github/workflows/ci.yml` — mỗi PR + push main: `backend-test` (JDK 21 temurin, `./mvnw test`, Testcontainers cần Docker có sẵn) + `frontend-test` (Node 20, `npm ci`, `npm run lint`, `npm run typecheck`, `npm test`). `.github/workflows/e2e.yml` — `workflow_dispatch` toàn stack (boot compose + 4 service + gateway + Mailpit → `smoke-auth.sh run` → `E2E_FULL=1 npx playwright test` → stop; best-effort chưa chạy trên Linux CI).
13. (Sau) user-service; track upload thật qua UI/FE + CDN/auth-cache cho streaming; mở rộng P2 FE component (account/2FA page); (optional) nâng P3 lên Pact broker đầy đủ + verify `e2e.yml` trên Linux CI.
